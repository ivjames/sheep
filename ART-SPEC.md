...
`https://sparkle.lab980.com/tools/sprite-manager.html` — no sign-in, the link
is all you need. It shows every sprite above as a grid of frame slots — upload
a PNG per frame (or drag a batch of `name_i.png` files anywhere and they route
themselves), preview each animation live at its real fps, and track
completion. Uploads land in a shared store, so anyone with the link sees the
latest art immediately. **Export** packs everything uploaded into a correctly
formatted `sprites.png` + `sprites.json` — partial exports work fine. (Opened
as a local file instead, the tool stores uploads in your own browser only.)
