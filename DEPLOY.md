...
Target: **https://sheep.lab980.com** — served from the lab980 droplet
...
the deployment. Everything is driven by the operate CLI at `bin/sheep` —
...
> local port). This site has no app, so `sheep setup` writes its own static
> vhost instead — same DNS/doctl, security-headers, and certbot shape.

## One-time bring-up (on the droplet, as root)

```bash
git clone https://github.com/ivjames/sheep /var/www/sheep
ln -sf /var/www/sheep/bin/sheep /usr/local/bin/sheep
sheep setup
```

`sheep setup` is idempotent and does, in order:

1. **DNS** — `doctl` A record `sheep.lab980.com -> droplet IP` (skipped if
   it already exists; `--no-dns` to skip, `--ip` to override autodetect).
   Manual equivalent:
   `doctl compute domain records create lab980.com --record-type A --record-name sheep --record-data 165.22.128.19 --record-ttl 300`
2. **nginx** — static vhost from `deploy/nginx.conf.template` installed as
   `/etc/nginx/sites-available/sheep.lab980.com`, symlinked into
...
3. **TLS** — waits for DNS to resolve, then
   `certbot --nginx -d sheep.lab980.com --redirect -n`. If DNS is still
...
```bash
sheep status         # HEAD commit, live probe, cert days
health-check --site sheep    # the droplet-wide auditor also covers it
```

## Overrides

- `SHEEP_FQDN` — serve under a different name (default `sheep.lab980.com`)
- `SHEEP_BRANCH` — deploy a different branch (default `main`)
